...
$$\theta^{(i+1)} & = argmax_{\theta}(L(\theta^{(i)}) + \sum_Z P(Y|Z, \theat^{(i)}) log(\frac {P(Y|Z,\theta)P(Z|\theta)}{P(Y|Z, \theta^{(i)})P(Y|\theta^{(i)})})) \\
& = argmax_{\theta}(\sum_Z P(Y|Z, \theta^{(i)}) log(P(Y|Z,\theta)P(Z|\theta))) \\
& = argmax_{\theta}(\sum_Z P(Y|Z, \theta^{(i)}) log(P(Y,Z|\theta)))=argmax_{\theta}Q(\theta, \theta^{(i)})) \tag{7}$$
